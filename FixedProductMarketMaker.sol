...
contract FixedProductMarketMaker is ERC20, ERC1155TokenReceiver {
    ...
    function getFee() public view returns (uint) {
        if (block.timestamp >= endTime) return maxFee;
        if (block.timestamp <= startTime) return fee;
        
        uint totalDuration = endTime - startTime;
        if (totalDuration == 0) return maxFee;

        uint timePassed = block.timestamp - startTime;
        uint diff = maxFee - fee;
        
        // (timePassed / totalDuration)^4
        // Calculate with 18 decimals precision
        uint factor = timePassed.mul(ONE) / totalDuration;
        uint factor2 = factor.mul(factor) / ONE;
        uint factor4 = factor2.mul(factor2) / ONE;
        
        return fee.add(diff.mul(factor4) / ONE);
    }
    ...
    function getBNBFee(uint amount) public view returns (uint) {
        if (address(uniswapRouter) == address(0)) return 0;
        if (wbnb == address(collateralToken)) {
            return amount.mul(protocolFeeBps).div(10000);
        }
        address[] memory path = feePath;
        uint[] memory amounts = IUniswapV2Router(uniswapRouter).getAmountsOut(amount, path);
        return amounts[amounts.length - 1].mul(protocolFeeBps).div(10000);
    }
    ...
    function calcBuyAmount(uint investmentAmount, uint outcomeIndex) public view returns (uint) {
        ...
        uint currentFee = getFee();
        uint[] memory poolBalances = getPoolBalances();
        uint investmentAmountMinusFees = investmentAmount.sub(investmentAmount.mul(currentFee) / ONE);
    ...
    function calcSellAmount(uint returnAmount, uint outcomeIndex) public view returns (uint outcomeTokenSellAmount) {
        ...
        uint currentFee = getFee();
        uint[] memory poolBalances = getPoolBalances();
        uint returnAmountPlusFees = returnAmount.mul(ONE) / ONE.sub(currentFee);
    ...
    function buy(uint investmentAmount, uint outcomeIndex, uint minOutcomeTokensToBuy) external payable {
        uint outcomeTokensToBuy = calcBuyAmount(investmentAmount, outcomeIndex);
        require(outcomeTokensToBuy >= minOutcomeTokensToBuy, "minimum buy amount not reached");

        if (address(uniswapRouter) != address(0)) {
            uint bnbFee = getBNBFee(investmentAmount);
            require(msg.value >= bnbFee, "insufficient BNB fee");
            protocolFeeRecipient.transfer(bnbFee);
            if (msg.value > bnbFee) {
                msg.sender.transfer(msg.value - bnbFee);
            }
        }
        ...
        uint currentFee = getFee();
        uint feeAmount = investmentAmount.mul(currentFee) / ONE;
    ...
    function sell(uint returnAmount, uint outcomeIndex, uint maxOutcomeTokensToSell) external payable {
        ...
        if (address(uniswapRouter) != address(0)) {
            uint bnbFee = getBNBFee(returnAmount);
            require(msg.value >= bnbFee, "insufficient BNB fee");
            protocolFeeRecipient.transfer(bnbFee);
            if (msg.value > bnbFee) {
                msg.sender.transfer(msg.value - bnbFee);
        ...
        uint currentFee = getFee();
        uint feeAmount = returnAmount.mul(currentFee) / (ONE.sub(currentFee));
        feePoolWeight = feePoolWeight.add(feeAmount);
...
contract FixedProductMarketMakerData {
    ...
    uint internal protocolFeeBps;
    address[] internal feePath;
